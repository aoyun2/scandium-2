...
function request() {
	return new Promise((success, fuckedup) => {   
		const options = {
		    host: 'api.tenor.com',
		    path: '/v1/search?q=anime+slap&key=TDTPDQSBTRRA',
		};
...
module.exports.run = async (bot,message,args) => {  
    ...
    {
	    await message.channel.send(`Cannot slap yourself.`);
	    return; 
    }
    ...
    const exampleEmbed = new Discord.MessageEmbed()
      .setColor('#A3A6E8')
      .setTitle(`**${message.author.username} slapped ${person.displayName}**`)
      .setImage(gif)
      .setTimestamp()
